    ...
    def query_knowledge_base(
        ...
        try:
            ...
            if knowledge_base_id == "local-dev-placeholder-kb-id":
                logger.warning(
                    "🛠️ Local development mode: Cannot query Knowledge Base without deployed infrastructure"
                )
...
Answer:"""
        ...
        try:
            ...
            return {
                ...
                "metadata": {
                    ...
                    "total_time": kb_result["query_time"]
    ...
    def _extract_location_info(self, location: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if location.get("type") == "S3":
            ...
            return {
                ...
                "bucket": (
                    s3_location.get("uri", "").split("/")[2]
                    if s3_location.get("uri")
                    else ""
                ),
                "key": (
                    "/".join(s3_location.get("uri", "").split("/")[3:])
                    if s3_location.get("uri")
                    else ""
                ),
            }
    ...
    def _create_local_dev_response(
